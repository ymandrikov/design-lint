...
let cache = { source: null, lang: null, ast: null };
...
export function parseSource(source, filePath = "file.tsx") {
  const lang = langForFile(filePath);
  if (cache.source === source && cache.lang === lang) return cache.ast;
  ...
  const ast = { program, comments, lineStarts: buildLineStarts(source), ignored: null };
...
export function ignoredLines(ast) {
...
export function classNameStatics(value) {
  const out = [];
  collectClassStatics(value, out);
  return out;
}

function collectClassStatics(node, out) {
  if (!node) return;
...
export function classNameStaticsDeep(value) {
  const out = [];
  walk(value, (n) => {
...
export function styleObjectProps(attrValue) {
...
function objectExpressionOf(attrValue) {
  if (!attrValue || attrValue.type !== "JSXExpressionContainer") return null;
  let e = attrValue.expression;
  while (
    e &&
